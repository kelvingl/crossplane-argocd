...
- **`gogs`** — `http://gitadmin:...@localhost:<porta>/gitadmin/platform.git`
  ...
  `bootstrap/scripts/05-push-to-gogs.sh` (via port-forward temporário durante o script).
...
(`bootstrap/scripts/11-push-dataplanes-repo.sh`): ele só existe no Gogs, sem um
`origin` GitHub equivalente documentado neste repo.

## Ordem de sync-wave: por que essa ordem

A árvore app-of-apps (descoberta por `root-app-of-apps` a partir de
`gitops/root/app-of-apps.yaml`, que aponta para `gitops/apps/*.yaml` e
`gitops/appset/*.yaml`) usa três waves:
...
- **wave `"1"`**: `crossplane-providers`, e as duas Compositions
  (`crossplane-compositions`, `crossplane-compositions-s3`). Precisam do
  Crossplane core (wave 0) já instalado; `crossplane-compositions`
  (`dataplane-cluster`) usa `provider-helm`, instalado na mesma wave; a
  Composition `s3-bucket` precisa do MiniStack (wave 0) já respondendo em
...
`gitops/appset/dataplanes-appset.yaml` usa o gerador git `files` apontando para
...
1. **fonte 1** — este repositório (`platform`), caminho
   `gitops/dataplanes-fanout/chart` (o chart que faz o fan-out — ver abaixo), com
   ...
   `gitops/dataplanes-fanout/chart/values.yaml`).
2. **fonte 2** — o repositório `dataplanes`, referenciado só como `ref: values`.

> **Nomes parecidos, coisas diferentes**: `gitops/dataplanes-fanout/chart/`
> (este chart, wrapper/fan-out, existe desde ADR-029 — renomeado de
> `charts/dataplane-cluster/` na reorganização de pastas) e
> `gitops/crossplane/compositions/dataplane-cluster/` (a Composition
> `XDataPlane` da feature 003, que cria o spoke em si) têm nomes-base
> parecidos por coincidência histórica — são artefatos completamente
> distintos, em pastas de topo diferentes.
...
via `gitops/dataplanes-fanout/chart/templates/definitions/dataplane-claim.yaml`, direto — não
...
- entradas de **`charts`**: `destination.name: <spoke>` — endereça o
  Cluster do ArgoCD registrado por `bootstrap/scripts/16-register-argocd-clusters.sh`
  diretamente, sem Crossplane no meio. Fonte: o próprio repo `dataplanes`,
  caminho da entrada (`charts/<nome>/chart`).
- entradas de **`compositions`**: `destination.server` é sempre o hub — o
  ...
  `gitops/dataplanes-fanout/chart/templates/_helpers.tpl` — nenhuma composition
...
4. Essa Application renderiza gitops/dataplanes-fanout/chart, que emite:
   - a claim DataPlane <spoke> (sempre)
   - uma Application filha por entrada de charts/compositions (se houver)
5. A claim DataPlane vira um Release do provider-helm
   (gitops/crossplane/compositions/dataplane-cluster), que instala o
   chart do vcluster no hub
6. bootstrap/scripts/16-register-argocd-clusters.sh registra o spoke novo
...
## O padrão `ApplicationSet` para `gitops/addons/` (instalado em todo spoke)

Diferente do padrão acima (um arquivo `dataplanes/<spoke>.yaml` editado por
spoke), o `ApplicationSet` `dataplane-addons`
(`gitops/appset/dataplane-addons-appset.yaml`) instala cada chart em
`gitops/addons/<nome>/` em **todo** spoke registrado no ArgoCD, sem nenhuma edição
...
1. `clusters`, com `selector.matchLabels: lab.example.org/role: dataplane` —
   ...
   mesmo `bootstrap/scripts/16-register-argocd-clusters.sh` que já cria o Secret do
   Cluster.
2. `git` `directories` sobre `gitops/addons/*`.
...
**Consequência prática**: adicionar `gitops/addons/<nome-novo>/` (um chart Helm
...
addon de exemplo `prometheus`) depende de `gitops/crossplane/compositions/dataplane-cluster`
...
Cada Composition em `gitops/crossplane/compositions/<nome>/` expõe o mesmo alvo de Makefile:
...
1. O arquivo mudado está sob um caminho que alguma `Application`/
   `ApplicationSet` referencia (`gitops/apps/*.yaml`/`gitops/appset/*.yaml`
   listam `source.path` ou `sources[].path` de cada uma)?
