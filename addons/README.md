...
Cada pasta aqui é um chart Helm instalado, **automaticamente, em todo
spoke registrado no ArgoCD**, pelo `ApplicationSet` `dataplane-addons`
(`gitops/apps/dataplane-addons-appset.yaml`) — um `matrix` generator
combinando o gerador `clusters` (filtrado por
`lab.example.org/role: dataplane`, para não incluir o `in-cluster`/hub) com
um gerador `git` `directories` sobre `addons/*`. O resultado é uma
...
Adicionar um addon novo = criar `addons/<nome>/` com um chart Helm válido;
ele aparece em todo spoke (existente ou futuro) no próximo sync, sem editar
o `ApplicationSet`.
...
- **`prometheus/`** — servidor Prometheus com sua UI web, exposta em
  ...
  fica alcançável de fora porque `compositions/dataplane-cluster` liga
  `sync.toHost.ingresses` no chart do vcluster — o objeto é sincronizado
